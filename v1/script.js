...
const modalData = {
    intro: {
        title: "<span class='fas fa-book-open' aria-hidden='true'></span> 노동조합, 규칙 만드는 곳?",
        content: `
    ...
    constitutionalBasis: {
        title: "<span class='fas fa-gavel' aria-hidden='true'></span> 우리의 힘! 헌법이 보장하는 권리",
        content: `
    ...
    collectiveAgreement: {
        title: "<span class='fas fa-file-signature' aria-hidden='true'></span> 단체협약: 우리 직장의 '법'",
        content: `
    ...
    representativeUnion: {
        title: "<span class='fas fa-bullhorn' aria-hidden='true'></span> 대표선수 노조의 역할",
        content: `
    ...
    localAutonomy: {
        title: "<span class='fas fa-city' aria-hidden='true'></span> 직장 규칙 vs 동네 규칙",
        content: `
    ...
    nationalAssembly: {
        title: "<span class='fas fa-landmark' aria-hidden='true'></span> 노조 활동, 국회랑 비슷해?",
        content: `
    ...
    foreignCases: {
        title: "<span class='fas fa-globe-americas' aria-hidden='true'></span> 다른 나라는 어떨까?",
        content: `
    ...
    limitations: {
        title: "<span class='fas fa-exclamation-triangle' aria-hidden='true'></span> 우리의 숙제: 현실의 벽",
        content: `
    ...
    strengtheningMeasures: {
        title: "<span class='fas fa-tools' aria-hidden='true'></span> 더 강한 노조! 어떻게?",
        content: `
    ...
    democracyRole: {
        title: "<span class='fas fa-balance-scale' aria-hidden='true'></span> 노조, 민주주의의 파트너!",
        content: `
...
const modalReturnBtn = modal.querySelector(".modal-return-btn"); // Query once
...
function openModal(targetId) {
    ...
    if (data) {
        previouslyFocusedElement = document.activeElement; // 1. Store the element that had focus before modal opened
        ...
        modalBodyContent.scrollTop = 0; // Scroll modal content to top

        modal.focus(); // 2. Set focus to the modal container (it has tabindex="-1" in HTML)
...
function closeModal() {
    ...
    setTimeout(() => {
        ...
        if (previouslyFocusedElement) {
            previouslyFocusedElement.focus(); // 3. Restore focus to the element that opened the modal
        }
    }, 300); // Duration should match CSS animation
}

// 4. Trap focus within the modal
function trapFocusInModal(event) {
    ...
    ).filter(el => el.offsetParent !== null); // Ensure elements are visible and focusable
...
// Add focus trapping listener to the modal
