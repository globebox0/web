...
const modalData = {
    ...
    representativeUnion: {
        ...
        content: `
            ...
            <p>결국, 막강한 힘과 무거운 책임을 가진 교섭대표노동조합이 얼마나 책임감을 갖고, 모든 동료들의 목소리를 소외 없이 골고루 담아내며 주도적으로 활동하느냐에 따라, 우리 모두를 위한 약속 즉, 우리 모두의 권익을 지켜줄 직장 규칙의 수준과 깊이가 결정된답니다.</p> 
            <p>우리가 가입할 노동조합을 선택하는 것은 '작은 국회'의 국회의원을 뽑는 것과 같아요. 정말 중요한 선택이죠?</p>
        `
    },
    localAutonomy: {
    ...
    bargainingUnificationProblems: {
        ...
        content: `
            ...
            <h4>하지만, 현실은 어떨까요?</h4>
            <ul>
                <li><strong>소수 노조의 설움:</strong> 교섭대표가 되지 못한 소수 노조는 단체교섭권이 없어서, 목소리를 내기 힘들어요. 이렇게 소수노조의 단체교섭권을 박탈하는 교섭창구단일화 제도에 대해 '과잉금지원칙 위반'이라 위헌이라는 헌법재판 소수의견도 있었어요. 5대 4로 아슬아슬하게 위헌 판단이 갈렸답니다.</li>
            ...
            <p>노동 3권의 핵심은 우리 스스로 근로조건을 결정하는 <strong>자기결정권</strong>과 <strong>집단적 자치</strong>예요. 현재 제도는 이를 제한할 수 있다는 비판을 받고 있지만, 물류 사업소 노동자들이 하나되어 힘을 합치면 우리의 목소리를 대변 할 수 있는 '교섭대표'를 만들어 낼 수 있어요.</p> 
            <p><strong>물류 사업소 노동자는 물류노조로 뭉치자!</strong> 이것이 우리의 권리를 찾고, 진정한 자치를 실현하는 첫 걸음이 될 수 있답니다.</p>
        `
    }
};
